...
- **OpenAI Codex（Codex 桌面）**：作为主要执行代理，用来读题、拆任务、改代码、跑测试、整理 README 和交付物。
- **Claude Code（Anthropic Claude）**：作为审核助手，用来审技术设计书、数据库 schema、API 设计、测试清单和前端设计方向。
...
竞品很复杂，但这次任务是 3 天全栈挑战，重点在 API、数据库、持久化、模拟订阅和测试质量。所以这里做了一些取舍。
...
BetterMe 前端里大量问题用数字 question id 存在 `answers.extra` 和 `rawAnswers`，灵活但不利于本题展示。我们最终没有把所有答案塞进一个 JSON 字段，而是采用“核心字段显式列 + 扩展题目/答案表”的混合模型。
...
这些字段直接参与 BMI、BMR/TDEE、目标日期和热量计算，必须能被 Zod、Prisma 类型、数据库关系和测试清楚约束。

扩展问卷字段不再继续堆在 `assessments` 主表，而是通过 `questionnaire_questions` 和 `assessment_answers` 保存：
...
这样做的原因是：核心字段保持强约束，扩展问题又可以通过题目定义表继续增加，避免“一张 JSON 大表”和“主表无限加列”两个极端。对于这次挑战来说，这比纯 JSON 更可解释，也比所有问卷字段都硬编码到主表更可扩展。
...
数据库建模阶段，我先让 AI 根据竞品数据流提出候选表结构，再按题目要求设计成 6 张关系表：
...
`assessments` 存用户分步填写的核心健康字段，以及 `step`、`completed` 和 `version`。
...
- `questionnaire_questions` 保存题目 key、文案、分组、值类型、排序和启用状态。
- `assessment_answers` 保存某次测评对某个题目的答案，按 `valueText`、`valueNumber`、`valueBoolean`、`valueJson` 受控落值。

这样后续新增生活方式、训练限制、饮食禁忌等非核心问题时，不需要继续修改 `assessments` 主表结构。API 仍然可以保持扁平 payload，后端内部把扩展字段映射到答案表。
...
测试阶段我没有让 AI 只写 happy path。我的做法是先把题目第四阶段拆成测试矩阵，再让 AI 按矩阵补测试数据和用例，最后用 Vitest 固化下来。

第四阶段要求至少证明四件事：

- 健康评估算法能处理边界、极端、缺失和非法输入。
- 分步保存和进度恢复能处理刷新、中断、乱序、重复提交和并发更新。
- 鉴权差异化返回必须能区分非会员脱敏结果和会员完整结果，非会员拿不到受保护字段。
- `/api/pay` 回调后，数据库状态和结果接口返回都要从“脱敏”变成“完整”。

所以我让 AI 先列风险点，再把风险点落到具体测试文件和具体 payload。这样测试不是为了凑数量，而是直接对应题目原文。
...
第四阶段还要求说明数据验证和一键运行方式，所以我让 AI 把接口校验类测试单独拎出来检查：

- 非法 UUID 提前返回 400，不进入数据库查询。
- 存在格式正确但不存在的 UUID 返回 404。
- enum 只能取设计范围内的值。
- 身高、体重、年龄、目标体重、训练天数、训练时长都有上下界。
- 数字字段传字符串注入或 `null` 时会被拒绝。

一键测试用 `npm test`，CI 用 GitHub Actions 自动跑 Prisma generate / migrate、lint、test 和 build。README 里也写清楚了暂未覆盖的部分：没有把完整浏览器 E2E 放进 `npm test`，没有接真实 Stripe，没有做 serverless 压测。原因是这道题的核心是后端闭环、数据持久化、订阅状态和接口边界，真实支付和压测属于后续生产化扩展。

## 7. AI 如何辅助项目管理文档维护

每一轮实现、审核、纠正和验证后，我会让 AI 把状态同步到项目管理文档，避免最后交付时靠记忆拼材料。
...
我否决了这个方案。原因很简单：题目要求的是“模拟订阅体系：鉴权、差异化返回、支付回调闭环”。如果完整数据一开始就到了前端，只靠 CSS blur 或按钮状态遮住，那只是视觉 paywall，不是后端闭环。
...
- `assessments` 存核心健康字段、step、completed、version。
- `questionnaire_questions` 存扩展题目定义。
- `assessment_answers` 存某次测评对扩展题目的答案。
- `results` 存服务端计算后的结果。
- `subscriptions` 存模拟订阅状态。

这样既保留了核心字段的强约束，也给非核心问卷问题留下了可扩展空间。它比全塞 JSON 更容易审查和测试，也比把所有问题都加成 `assessments` 列更经得起后续扩展。
