...
- 健康算法必须有单元测试。BMI、BMR/TDEE、目标体重和热量建议是结果页的基础，如果这些边界错了，后面的持久化和订阅都没有意义。
- 分步保存和进度恢复必须有集成测试。这个产品不是一次性提交表单，用户会中断、刷新、重复点按钮，也可能在旧页面上继续提交，所以测试覆盖了恢复、乱序、重复和并发版本冲突。
- 鉴权差异化和 `/api/pay` 必须端到端验证。非会员和会员的区别不能只靠前端模糊处理，必须由结果接口返回不同结构，并且 `/api/pay` 后同一个 `sessionId` 能从脱敏结果变成完整结果。
- 数据验证必须挡住非法输入。接口用 Zod 校验，测试覆盖 enum、越界数字、缺失字段、非法 UUID、数字注入和 `null` 数值，避免脏数据进入计算和数据库。

一键运行方式和 CI 见第四节：本地用 `npm test`，GitHub Actions 会在 push / pull request 时自动跑迁移、lint、测试和 build。
...
- 完整浏览器 E2E 暂未纳入 `npm test`。原因是题目重点在后端闭环和接口质量，当前页面流程用 Playwright CLI 做本地 smoke，避免为了演示项目引入更重的 E2E 基建。
- 真实支付 provider 未覆盖。原因是本题要求的是模拟订阅体系，当前只实现 `/api/pay` 回调闭环，不接真实 Stripe。
- Vercel serverless 压测暂未覆盖。原因是 3 天挑战的评分重点不是性能压测，当前只对关键 `/api/pay` 线上链路做人工可重放验证。
