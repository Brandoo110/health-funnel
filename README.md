...
演示环境里没有接真实支付通道。用户在结果页点击 `Get my plan` 时，前端会直接调用 `POST /api/pay`，这一步就代表“模拟支付成功”，然后当前 `sessionId` 被解锁为已支付状态。

前端的转化设计也围绕这个闭环展开：

- 初始 paywall 展示 7 折价格，并用倒计时表达“折扣仍被保留”，不是倒计时结束就不能购买。
- 用户刷新、关闭或点击“不确定”时，展示挽留弹窗，提供更强的 5 折 offer。
- 5 折 offer 按当前 `sessionId` 持久化；同一个报告刷新后仍保留 5 折价格，不会回退到 7 折。
- 换新 session 或重新开始测评时清理旧 offer，避免不同用户/不同报告串用折扣状态。

折扣只影响前端展示和模拟购买套餐选择，不改变后端的权限边界。真正是否能看到完整结果，仍然只由 `/api/pay` 写入后的订阅状态决定。
